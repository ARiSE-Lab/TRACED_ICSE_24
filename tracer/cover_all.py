...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", type=Path, help="program input directory", default=Path("../all_input_output"))
    parser.add_argument("--keep", action="store_true", help="whether to keep the temp dir after running")
    ...
    base_dir = Path("../Project_CodeNet").absolute()
    metadata_dir = (base_dir/"metadata")
    new_src_dir = (base_dir/"data")

    tmpdirname = tempfile.mkdtemp()
    try:
        tmp_dir = Path(tmpdirname)
        print("running in", tmp_dir)
        os.chdir(tmp_dir)
        out_dir = tmp_dir/"instrumented_output"
        exe_dir = tmp_dir/"instrumented_exes"
        out_metadata_dir = tmp_dir/"instrumented_metadata"
        out_dir.mkdir()
        exe_dir.mkdir()
        out_metadata_dir.mkdir()
        ...
        output_filename = cur_dir / "output.tar"
        with tarfile.open(output_filename, "w") as tar:
            tar.add(out_dir, arcname=out_dir.name)
            tar.add(exe_dir, arcname=exe_dir.name)
            tar.add(out_metadata_dir, arcname=out_metadata_dir.name)
    finally:
        if not args.keep:
