...
def parse_args():
    ...
    parser.add_argument(
        "--root_dir", default="/work/LAS/weile-lab/benjis/weile-lab/tracing"
    )
...
df["log_file"] = df.apply(
    lambda row: log_files[
        (row["lang"], row["problem"], row["solution"], row["input_id"])
    ],
    axis=1,
)
df["output_file"] = df.apply(
    lambda row: output_files[
        (row["lang"], row["problem"], row["solution"], row["input_id"])
    ],
    axis=1,
)
df["input_file"] = df.apply(
    lambda row: input_dir / row["problem"] / f'input_{row["input_id"]}.txt', axis=1
)
df["src_file"] = df.apply(
    lambda row: src_dir
    / row["problem"]
    / lang_to_path[row["lang"]]
    / (row["solution"] + "." + row["lang"]),
    axis=1,
)
...
with open(sequences_filename) as f:
    for line in tqdm.tqdm(f.readlines()):
        try:
            ...
            sequences_inputs.add(
                os.path.basename(sequence["filepath"]) + sequence["input_no"]
            )
...
choptop_all_trace_lengths = choptop_all_trace_lengths[
    : int(len(choptop_all_trace_lengths) * 0.99)
]
...
print(
    f"average trace length except 99th percentile: {np.average(choptop_all_trace_lengths):.4f}"
)
