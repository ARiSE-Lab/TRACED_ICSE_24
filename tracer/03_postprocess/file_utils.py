            ...
            with open(problem_index) as f:
                ...
                while line:
                    ...
                    if args.test and i >= 1000:
                        return
                    if (
                        args.limit_solutions is not None
                        and num_solutions >= args.limit_solutions
                    ):
                        break
        else:
            num_solutions = 0
...
def get_log_files(args):
    ...
    if args.begin is not None or args.end is not None:
        ...
        problem_list = [
            ("p" + str(i).rjust(5, "0")) for i in range(args.begin, args.end + 1)
        ]
    ...
    for base_dir in args.base_dirs:
        ...
        for output_file in tqdm.tqdm(
            get_files(args, base_dir / "outputs", problem_list)
        ):
            if args.test:
                assert output_file.exists(), log_file
            try:
                lang, problem, solution, input_str, input_id = output_file.stem.split(
                    "_"
                )
                run_id = (lang, problem, solution, input_id)
