...
def get_sequence_all_inputs(lang, problem, solution):
    ...
    if len(input_files) == 0:
        return []
    sequences_to_return = []
    for input_file in sorted(input_files, key=lambda f: f.stem if f is not None else None):
        ...
        for base_dir in base_dirs:
            ...
            output_file = base_dir / problem / lang / solution / f"{input_id}.txt_stdout.txt"
...
full_sequences_filename = sequences_filename.parent / (sequences_filename.stem + "_full" + sequences_filename.suffix)
with open(sequences_filename, 'w') as sf, open(full_sequences_filename, 'w') as full_sf:
    for problem_num in range(args.begin_problem, args.end_problem+1):
        ...
        num_success = 0
        with tqdm.tqdm(map(get_sequence_tuple, zip(df["language"], df["problem_id"], df["submission_id"])), total=len(df)) as pbar:
            for i, sequences in enumerate(pbar):
                ...
                for sequence in sequences:
                    ...
                    if sequence["outcome"] not in ["success_short_trace", "missing_log", "parse_error"]:
